...
class PhysicalModelSnippet {
  ...
  def deleteTable(): JsCmd = {
    if(SelectedTable.is != null) {

    	val pt = SelectedTable.is
    	PAttribute.findAll(By(PAttribute.fkPTable, pt.id)).map(_.delete_!)
    	pt.delete_!
    	SelectedTable(null)
    	RedirectTo("attribute")
    }
    else Alert(S.?("noTableSelection"))
  }
  ...
  def work(xhtml: NodeSeq): NodeSeq = {
    ...
    val deleteButton = if (MyUtil.isDesigner()) ajaxButton(S.?("deleteTable"), deleteTable _) % ("class" -> "standardButton") else ajaxButton(S.?("deleteTable"), empty _) % ("class" -> "standardButton") % ("disabled" -> "")
