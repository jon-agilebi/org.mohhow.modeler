...
object MeasureForm extends LiftScreen {
 ...
 val formula = textarea(S.?("formula"), nvl(SelectedMeasure.is.formula.toString), checkTerm _, "class" -> "termEditor")
 val mockup = textarea(S.?("mockup"), nvl(SelectedMeasure.is.mockup.toString), checkTerm _, "class" -> "termEditor")
